...
module.exports = (bookingSrv, itemsSrv, timeSlotsSrv, personSrv) => {
  ...
  router.get(
    ...
    asyncMiddleware(async (req, res, next) => {
      ...
      const ts = await timeSlotsSrv.getBookingTimeSlots(b.get('BuchungKey'));
      const eq = await itemsSrv.getBookedEquipment(b.get('BuchungKey'));
      ...
      const contract = {
        ...
        person: {
          ...
          address:
            p.get('Strasse') + ' ' + p.get('Hausnummer') + '/' + p.get('Top'),
          postCode: p.get('PLZ'),
          city: p.get('Ort')
        },
        timeSlots: ts.map(t => {
